...
/*   Updated: 2025/01/15 18:46:49 by thine            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_case_rrarb_pa(t_stack *a, t_stack *b, int nbr)
{
	...
	if (ft_search_insert_position_a(a, nbr))
		i = ft_lstsize(a) - ft_search_insert_position_a(a, nbr);
	i += ft_index(b, nbr);
	return (i);
}
...
int	ft_case_rrarrb_pb(t_stack *a, t_stack *b, int nbr)
{
	...
	i = 0;
	if (ft_search_insert_position_a(a, nbr))
		i = ft_lstsize(b) - ft_search_insert_position_a(a, nbr);
	if (a->nbr != nbr && i < (ft_lstsize(a) - ft_index(a, nbr)))
